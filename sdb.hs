...
usageText this = mapM_ err
    ...
    , "    psql              connect to snowblog_development with psql"
    ]
...
shakeit dbdir pghost pgdata = shakeArgs shakeOptions $ do
    ...
    phony "psql" $ do
        need [dbRunning]
        command [] "psql" ["snowblog_development"]

exportDb :: FilePath -> Shell ()
exportDb dbdir = do
    step "Dumping to devDB.sql..."
    mktree (dbdir </> "dev")
    output (dbdir </> "dev" </> "devDB.sql") $ pgDump "snowblog_development"
  where
...
initCluster pghost pgdata = do
    ...
    procs "createdb" ["snowblog_development"] empty
    procs "createdb" ["snowblog_test"] empty
